...
/// (`sendImageToA`/`sendFileToA`, hosted-URL media) and asserts the media bubble/filename arrives. Covers
/// 1TO1-089..093, E2E-021/022/073..077 and GRP-032..038 (group media).
final class MediaMessagesTests: XCTestCase {
    ...
    private func openAttachmentSheet() -> Bool {
        let byLabel = ["Attach", "Add", "Attachment", "Plus", "add"].compactMap { label -> XCUIElement? in
            let b = app.buttons[label]; return b.exists ? b : nil
        }.first
        if let b = byLabel { b.tap() }
        else {
