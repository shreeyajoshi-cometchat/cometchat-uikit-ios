...
/// keyed by the acting user's role. Covers permission cases
/// GRP-026/028/029/031. These carry real regression value: they prove the framework hides Edit/Delete
/// for messages the current user isn't allowed to modify, and shows Delete for admins/moderators.
///
...
/// - participant  → cannot edit or delete B's message (GRP-026, GRP-029)
/// - admin/owner  → can delete B's message (GRP-028)
/// - moderator    → can delete B's message (GRP-031)
///
...
final class GroupMessagePermissionTests: XCTestCase {
    ...
    private func openGroupWithBMessage(aScope: String) -> String {
        ...
        let context: SeedData.TestGroupContext? = try? runBlocking {
            ...
            _ = try await PeerActions.sendGroupTextMessage(token, groupId: c.group.guid)
            return c
        }
        ctx = context
