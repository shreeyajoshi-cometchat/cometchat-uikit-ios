...
/// Joining a password-protected group. Covers join cases GRP-011/012.
...
final class GroupJoinTests: XCTestCase {
    ...
    override func tearDownWithError() throws {
        ...
        runBlocking { if let g { await PeerActions.deleteGroup(guid: g.guid, owner: TestConfig.userBUid) } }
    }
    ...
    func test_GRP_joinPasswordGroupCorrect() throws {
        ...
        guard let g = group else { return XCTFail("no group") }
        let joined = waitForBackend(timeout: 15) {
            let members = try await PeerActions.groupMemberUIDs(guid: g.guid, as: nil)
    ...
    func test_GRP_joinPasswordGroupWrong() throws {
        ...
        guard let g = group else { return XCTFail("no group") }
        let members = (try? runBlocking { try await PeerActions.groupMemberUIDs(guid: g.guid, as: nil) }) ?? []
    ...
    private func openJoinSheet() {
        let g = try? runBlocking { try await SeedData.createPasswordGroupWithoutA(password: Self.password) }
        group = g
        XCTAssertNotNil(g, "Could not create the password group")
        ...
        search.tap(); search.typeText(g!.name)
        let cell = app.cells.containing(.staticText, identifier: g!.name).firstMatch
