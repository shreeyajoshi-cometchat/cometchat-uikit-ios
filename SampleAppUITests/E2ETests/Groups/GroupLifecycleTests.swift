...
/// banned-members list + unban. Covers high-value cases GRP-068/075/076/080.
...
final class GroupLifecycleTests: XCTestCase {
    ...
    override func tearDownWithError() throws {
        ...
        let c = ctx; ctx = nil
        runBlocking {
            await SeedData.deleteTestGroup(g)
            await SeedData.deleteTestGroup(c)
        }
    }
    ...
    // MARK: - Transfer ownership (GRP-080)
    ...
    func test_GRP_transferOwnership() throws {
        ...
        let transferred = waitForBackend(timeout: 20) {
            let scope = try await PeerActions.memberScope(guid: g.guid, uid: TestConfig.userBUid, as: nil)
    ...
    func test_GRP_addMemberJoinsGroup() throws {
        ...
        let added = waitForBackend(timeout: 20) {
            let members = try await PeerActions.groupMemberUIDs(guid: g.guid)
    ...
    func test_GRP_unbanMember() throws {
        ...
        let bannedBefore = try runBlocking { await PeerActions.isGroupMemberBanned(guid: g.guid, uid: g.memberUid) }
        ...
        let unbanned = waitForBackend(timeout: 15) {
            !(await PeerActions.isGroupMemberBanned(guid: g.guid, uid: g.memberUid))
        }
    ...
    private func tapCard(_ label: String) {
        ...
        XCTAssertTrue(el.waitForExistence(timeout: 8), "\(label) card/action not found")
